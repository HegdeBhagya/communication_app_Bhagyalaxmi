...
const setLoggedInUserEmail = (email) => localStorage.setItem(LOGGED_IN_KEY, email);
...
const addUpload = () => {
  ...
  const newUpload = {
    id: loadUploads().length > 0 ? loadUploads()[loadUploads().length - 1].id + 1 : 1,
    label: label,
    fileName: fileName,
...
const initializePage = () => {
  ...
  const excludeNavPages = ["register.html", "login.html", "registerSuccess.html", "logout.html", "welcome.html"];
